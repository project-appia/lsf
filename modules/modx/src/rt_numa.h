...
static void rt_numa_set_numa_run_on_node(int node, int cpu)
{
	...
	if (res)
        std::cout << "Could not set NUMA node %d for thread " <<  node << " " << cpu << ":" << strerror(errno) << std::endl;
	return;
}
...
static void *rt_numa_numa_alloc_onnode(size_t size, int node, int cpu)
{
	...
	if (stack == NULL)
        std::cout << "failed to allocate " << size << "bytes on node " << node << " for cpu " << cpu << std::endl;
        return stack;
}
...
static int rt_numa_numa_node_of_cpu(int cpu)
{
	...
	if (node == -1)
                std::cout << "invalid cpu passed to numa_node_of_cpu(" << cpu << ")" << std::endl;
	return node;
}
...
static void numa_on_and_available()
{
    ...
    if (numa_available() == -1)
        std::cout << "--numa specified and numa functions not available." << std::endl;
}
